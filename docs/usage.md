...
```bash
nextflow pull remiolsen/nf-core-radseq
```

### Reproducibility
It's a good idea to specify a pipeline version when running the pipeline on your data. This ensures that a specific version of the pipeline code and software are used when you run your pipeline. If you keep using the same tag, you'll be running the same version of the pipeline, even if there have been changes to the code since.

First, go to the [nf-core/radseq releases page](https://github.com/remiolsen/nf-core-radseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
    ...
    * Runs using the `local` executor and pulls software from dockerhub: [`remiolsen/nf-core-radseq`](http://hub.docker.com/r/remiolsen/nf-core-radseq/)
* `awsbatch`
